...
- Nothing here has run. It is a specification, not a working plugin — no hooks, no commands, no scripts exist.
...
- The nag-once mechanism needs somewhere to record that a report was delivered. The obvious place is HANDOFF front matter, but that file is write_once and freezes at session end — so a SessionStart write to it would violate its own discipline. UNRESOLVED, and the first thing to settle before implementing.
